...
# Increase Java Min/Max Heap to maximus of available RAM

#how much ram can be used for Solr / Java VM
...
# if memory > 1400 MB set maximal usable memory to 768 MB
if [ $MEM -gt 1400 ]
then
	MAXMEM="768m"
fi

# if memory > 1900 MB set maximal usable memory to memory-1GB (for the system)
if [ $MEM -gt 1900 ]
then
	let MAXMEM=MEM-1024
	MAXMEM="${MAXMEM}m"
...
# if memory > 2900 MB set maximal usable memory to memory-1.5 GB (for the system)
if [ $MEM -gt 2900 ]
then
	let MAXMEM=MEM-1524
	MAXMEM="${MAXMEM}m"
...
# if memory > 3800 MB set maximal usable memory to memory-2 GB (for the system)
if [ $MEM -gt 3800 ]
then
	let MAXMEM=MEM-2000
	MAXMEM="${MAXMEM}m"
...
SOLR_JAVA_MEM="-Xms512m -Xmx${MAXMEM}"
